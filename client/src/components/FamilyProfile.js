...
const FamilyProfile = () => {
  ...
  const fetchFamilyProfile = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/auth/family-profile/${userId}`);
  ...
  const handleCreateAvatar = async (e) => {
    ...
    if (newAvatarName.trim() && newAvatarImage) {
      try {
        ...
        const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/auth/add-avatar`, formData, {
